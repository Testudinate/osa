...
class RemoveDuplication(object):
    ...
    def remove_duplication(self):
        ...
        sql = """DROP TABLE IF EXISTS {schemaName}.ANL_RULE_ENGINE_STAGE_FACT_DUPLICATION{suffix};
        ...
        sql = """UPDATE /*+ DIRECT, label(GX_OSM_RULE_ENGINE)*/ ANL_RULE_ENGINE_STAGE_FACT_TARGET_RULE_SET a
            ...
            FROM {schemaName}.ANL_RULE_ENGINE_STAGE_FACT_DUPLICATION{suffix} b
            WHERE a.retailer_key = b.retailer_key
            AND a.vendor_key = b.vendor_key AND a.id = b.id"""\
