#sql_tmp = _get_sublevel_filter.process() - verticaConn $verticaConn - schemaName $schemaName - providerSubType $providerSubType
# - ruleId $ruleId - ruleSetID $ruleSetID - subLevelColumn $subLevelMetrics - condition $sqltmp1 - rejectReason $metricsRejectReason - filterName $filterName
...
class GetSQLSubLevelFilter(object):
    ...
    def process(self, provider_subtype, rule_id, rule_set_id, sub_level_column, condition, reject_reason, filter_name):
        print("\n-------------- Calling GetSQLSubLevelFilter class --------------")
        # return SQL for sub level filter
        # param($verticaConn,$schemaName,$providerSubType,$ruleId,$ruleSetID,$subLevelColumn,$condition,$rejectReason,$filterName)
        ...
        for row in _result_sets:
            ...
            _subLevelValue = row.sub_level_value.replace("'", "''")
            _sqlToReturn += " WHEN {subLevelColumn} = '{subLevelValue}' THEN " \
                            "CASE WHEN {newCondition} THEN '{rejectReason}' ELSE '1' END"\
                .format(subLevelColumn=sub_level_column, newCondition=_new_condition,
                        subLevelValue=_subLevelValue, rejectReason=reject_reason)
            loop_times += 1
