...
#$verticaConn $sqlConn $hubConn {schemaName} $hubID -inaccurateAlertsDys $parameter1
...
# param($verticaConn,$sqlConn,$hubConn,$schemaName,$hubID,$inaccurateAlertsDys,$notInPlanogramDays,$trueAlertDays,$ruleSetId,$ruleID,$siloID)
    def __init__(self, conn, context, inaccurate_alerts_dys, notin_planogram_days, true_alert_days, rule_set_id, rule_id):
        ...
        self._notin_planogram_days = notin_planogram_days
    ...
    def process(self):
        ...
        sql="INSERT /*+ DIRECT, label(GX_OSM_RULE_ENGINE)*/ INTO {schemaName}.ANL_RULE_ENGINE_STAGE_SUB_LEVEL_FILTER_1{suffix} "\
            ...
            "SELECT 'default', 'TrueAlert', '{trueAlertDays}';".format(schemaName=self._schema_name,
                                                                      ...
                                                                      notInPlanogramDays=self._notin_planogram_days,
        ...
        sql="SELECT /*+label(GX_OSM_RULE_ENGINE)*/ COUNT(*) FROM {schemaName}.ANL_RULE_ENGINE_SUB_LEVEL_FILTER " \
            ...
            "AND SUB_LEVEL_CATEGORY = 'Alert Type';".format(schemaName=self._schema_name,
                                                           ruleSetId=self._rule_set_id,
                                                           ruleID=self._rule_id)
        ruleExists=self.dw_connection.query_scalar(sql)[0]
        if ruleExists != 0:
            sqlForFeedback = \
                "SELECT fdbk.Store_Id, fdbk.vendor_key, fdbk.Inner_UPC, fdbk.STORE_VISIT_DATE, fdbk.FEEDBACK_DESCRIPTION, " \
                "      COALESCE(sublevel.SUB_LEVEL_VALUE,'default') AS OSM_MAJOR_CATEGORY, fdbk.Merchandiser,fdbk.Retailer_Key, " \
                ...
                "ON fdbk.ALERT_ID = alert.IncidentID and fdbk.RETAILER_KEY = alert.RETAILER_KEY and fdbk.VENDOR_KEY = alert.VENDOR_KEY " \
